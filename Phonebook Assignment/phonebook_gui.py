...
def load_gui(self):
    ...
    # The TEXTBOXES:
    # 'Entry' == tkinter widget: a single-line textbox accepting input; 'self.master' is the ENTIRE window
    self.txt_fname = tk.Entry(self.master, text='') 
    self.txt_fname.grid(rox=1, column=0, rowspan=1, columnspan=2, padx=(30,40), pady=(0,0), sticky=N+E+W)
    ...
    # The LISTBOX & SCROLLBAR:
    self.scrollbar1 = Scrollbar(self.master, orient=VERTICAL)
    self.lstList1 = Listbox(self.master, exportselection=0, yscrollcommand=self.scrollbar1.set)
    self.lstList1.bind('<<ListboxSelect>>', lambda event: phonebook_func.onSelect(self, event))
    self.scrollbar1.config(command=self.lstList1yview) # 'config' allows changing something DYNAMICALLY (while a program is running)
    self.scrollbar1.grid(row=1, column=5, rowspan=7, padx=(0,0), pady=(0,0), sticky=N+E+S)
    ...
    self.btn_close.tkButton(self.master, width=12, height=2, text='Close', command=lambda: phonebook_func.ask_quit(self))
